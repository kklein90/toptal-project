...
echo "$(ts) - getting initial web env file from ssm" | tee -a $LOG
aws --region us-east-1 ssm get-parameters --with-decryption --names /app/web --query "Parameters[*].Value" --output text > /home/admin/toptal-project/node-3tier-app2/web/.env

echo "$(ts) - starting the web server" | tee -a $LOG
sudo -H -u admin bash -c 'docker-compose -f /home/admin/toptal-project/node-3tier-app2/web/docker-compose.yml up -d'
